...
export default class Client {
    ...
    public constructor(server?: Server | FakeServer) {
        ...
        if (server instanceof Server) {
            ...
            this.nextcloudAuthHeader =
                "Basic " + Buffer.from(server.basicAuth.username + ":" + server.basicAuth.password).toString("base64")
            this.nextcloudRequestToken = ""
            if (server.url.slice(-1) === "/") {
    ...
    public async getQuota(): Promise<IQuota> {
        ...
        const response: Response = await this.getHttpResponse(this.webDAVUrl + "/", requestInit, [207], {
            description: "Client get quota"
        })

        const properties: any[] = await this.getPropertiesFromWebDAVMultistatusResponse(
            response,
    ...
    public async getTags(): Promise<Tag[]> {
        ...
        const response: Response = await this.getHttpResponse(this.nextcloudOrigin + relUrl, requestInit, [207], {
    ...
    public async getFileId(fileUrl: string): Promise<number> {
        ...
        const response: Response = await this.getHttpResponse(fileUrl, requestInit, [207], {
    ...
    public async getFolderContents(folderName: string): Promise<any[]> {
        ...
        const response: Response = await this.getHttpResponse(url, requestInit, [207], {
    ...
    public async moveFolder(sourceFolderName: string, tarName: string): Promise<Folder> {
        ...
        try {
            ...
            throw new ClientError(
                `Error: moving folder failed: source=${sourceFolderName} target=${tarName} - ${err.message as string}`,
...
*/
    ...
    public async getUserGroupIds(search?: string, limit?: number, offset?: number): Promise<string[]> {
        ...
        const response: Response = await this.getHttpResponse(url, requestInit, [200], {
    ...
    public async createUserGroup(id: string): Promise<UserGroup> {
        ...
        const response: Response = await this.getHttpResponse(this.getOcsUrl(`/groups`), requestInit, [200], {
        ...
        throw new OperationFailedError(
            `User group ${id} could not be created: ${this.getOcsMetaStatus(rawResult).message}`
        )
    }
    ...
    public async deleteUserGroup(id: string): Promise<void> {
        ...
        log.debug("request body: ", requestInit.body)
        const response: Response = await this.getHttpResponse(this.getOcsUrl(`/groups/${id}`), requestInit, [200], {
    ...
    public async createUser(options: { id: string; email?: string; password?: string }): Promise<User> {
        ...
        log.debug("request body: ", requestInit.body)
        const response: Response = await this.getHttpResponse(this.getOcsUrl(`/users`), requestInit, [200], {
        ...
        throw new UserCreateError(
            `Error creating user '${options.id}' - ${this.getOcsMetaStatus(rawResult).message} (${
    ...
    public async updateUserProperty(id: string, property: UserProperty, value: string): Promise<void> {
        ...
        const url = this.getOcsUrl(`/users/${id}`)
        log.debug("request body: ", requestInit.body)
        const response: Response = await this.getHttpResponse(url, requestInit, [200, 401], {
        ...
        */
        ...
        throw new UserUpdateError(
            `User with id '${id}' could not be updated - ${property}=${value}. ${rawResult.ocs.meta.message as string}`
        )
    }
    ...
    public async addUserToMemberUserGroup(id: string, userGroupId: string): Promise<void> {
        ...
        throw new OperationFailedError(
            `User ${id} could not be added to user group ${userGroupId}: ${this.getOcsMetaStatus(rawResult).message}`
        )
    }
    ...
    public async createShare(options: ICreateShare): Promise<Share> {
        ...
        // try {
        const response: Response = await this.getHttpResponse(url, requestInit, [200, 204], {
            description: "Share create"
        })

        const rawResult: { ocs: { data: { id: string } } } = (await response.json()) as {
            ocs: { data: { id: string } }
        }
    ...
    public async updateShare(
        shareId: string,
        body: { password: string } | { expireDate: string } | { note: string } | { permissions: number }
    ): Promise<void> {
    ...
    */
    ...
    public async sendNotificationToUser(userId: string, shortMessage: string, longMessage?: string): Promise<void> {
        ...
        const queryString = `${encodeURIComponent(userId)}?shortMessage=${encodeURIComponent(
            shortMessage
        )}${longMessage}`
        await this.getHttpResponse(
    ...
    public async getApps(): Promise<string[]> {
        ...
        const response: Response = await this.getHttpResponse(this.getOcsUrl(`/apps`), requestInit, [200], {
    ...
    public async getAppInfos(appName: string): Promise<object> {
        ...
        const response: Response = await this.getHttpResponse(this.getOcsUrl(`/apps/${appName}`), requestInit, [200], {
